 ...
 * Send transaction
 *
 * @param params - Transaction parameters including recipient and amount
 * @returns Promise that resolves to send transaction result
 */
export async function sendTransaction(
  ...
  try {
    ...
    const recipientAddress =
      params.to ?? 'hoosat:qr1234567890abcdef1234567890abcdef12345678';
...
export async function testTxBuilder(): Promise<SendTransactionResult> {
  try {
    ...
    if (
      !utxosResult.success ||
      !utxosResult.data?.utxos ||
      utxosResult.data.utxos.length === 0
    ) {
      return {
        success: false,
